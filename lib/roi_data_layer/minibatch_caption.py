...
"""Compute minibatch blobs for training a Text-to-Clip Retrieval network."""
...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        # gt captions: (input_sentence, cont_sentence, target_sentence)
        gt_captions = np.empty((len(gt_inds), 3, cfg.MAX_WORDS), dtype=np.float32)

        gt_captions[:, 0] = roidb[0]['input_sentence'][gt_inds,:]
        gt_captions[:, 1] = roidb[0]['cont_sentence'][gt_inds,:]
        gt_captions[:, 2] = roidb[0]['target_sentence'][gt_inds,:]
