 ...
 *     i_t := \sigmoid[ W_{hi} * h_{t-1} + W_{xi} * x_t + W_{p} * x_p + b_i ]
 *     f_t := \sigmoid[ W_{hf} * h_{t-1} + W_{xf} * x_t + W_{p} * x_p + b_f ]
 *     o_t := \sigmoid[ W_{ho} * h_{t-1} + W_{xo} * x_t + W_{p} * x_p + b_o ]
 *     g_t :=    \tanh[ W_{hg} * h_{t-1} + W_{xg} * x_t + W_{p} * x_p + b_g ]
...
class LSTMTopicLayer : public RecurrentLayer<Dtype> {
 public:
  explicit LSTMTopicLayer(const LayerParameter& param)
      : RecurrentLayer<Dtype>(param) {}

  virtual inline const char* type() const { return "LSTMTopic"; }
 ...
 protected:
  ...
  // LSTMTopicFirst: show topic at the first timestep.                                                                                                                                      
template <typename Dtype>
class LSTMTopicFirstLayer : public RecurrentLayer<Dtype> {
  public:
    explicit LSTMTopicFirstLayer(const LayerParameter& param)
      : RecurrentLayer<Dtype>(param) {}

    virtual inline const char* type() const { return "LSTMTopicFirst"; }
...
// show x_static only at the first timestep
template <typename Dtype>
class LSTMFirstLayer : public RecurrentLayer<Dtype> {
 public:
  explicit LSTMFirstLayer(const LayerParameter& param)
      : RecurrentLayer<Dtype>(param) {}

  virtual inline const char* type() const { return "LSTMFirst"; }
...
template <typename Dtype>
class LSTMControllerLayer : public RecurrentLayer<Dtype> {
 public:
  explicit LSTMControllerLayer(const LayerParameter& param)
      : RecurrentLayer<Dtype>(param) {}

  virtual inline const char* type() const { return "LSTMController"; }
...
//template <typename Dtype>
//class LSTMUnit1Layer : public Layer<Dtype> {
// public:
//  explicit LSTMUnit1Layer(const LayerParameter& param)
//      : Layer<Dtype>(param) {}
//  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
//      const vector<Blob<Dtype>*>& top);
//
//  virtual inline const char* type() const { return "LSTMUnit1"; }
//  virtual inline int ExactNumBottomBlobs() const { return 3; }
//  virtual inline int ExactNumTopBlobs() const { return 2; }
//
//  virtual inline bool AllowForceBackward(const int bottom_index) const {
...
//
//template <typename Dtype>
//class LSTMUnit2Layer : public Layer<Dtype> {
// public:
//  explicit LSTMUnit2Layer(const LayerParameter& param)
//      : Layer<Dtype>(param) {}
//  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
//      const vector<Blob<Dtype>*>& top);
//
//  virtual inline const char* type() const { return "LSTMUnit2"; }
//  virtual inline int ExactNumBottomBlobs() const { return 3; }
//  virtual inline int ExactNumTopBlobs() const { return 2; }
//
//  virtual inline bool AllowForceBackward(const int bottom_index) const {
