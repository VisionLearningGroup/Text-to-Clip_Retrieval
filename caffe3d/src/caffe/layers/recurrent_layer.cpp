...
void RecurrentLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  topic_input_ = (bottom.size() > 3 + num_hidden_exposed);
  if (static_input_) {
  ...
  if (topic_input_) {
    ...
    input_layer_param->add_top("x_topic");
    input_param->add_shape()->CopyFrom(input_shape);
  }
  ...
  if (topic_input_) {
    x_topic_input_blob_ =
        CHECK_NOTNULL(unrolled_net_->blob_by_name("x_topic").get());
  }
  ...
  CHECK_EQ(2 + num_recur_blobs + static_input_ + topic_input_,
...
void RecurrentLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (topic_input_) {
    x_topic_input_blob_->ReshapeLike(*bottom[3]);
  }
  ...
  if (topic_input_) {
    x_topic_input_blob_->ShareData(*bottom[3]);
    x_topic_input_blob_->ShareDiff(*bottom[3]);
  }
  if (expose_hidden_) {
    const int bottom_offset = 2 + static_input_ + topic_input_;
