...
void LSTMTopicLayer<Dtype>::RecurrentInputBlobNames(vector<string>* names) const {
  names->resize(2);
  (*names)[0] = "h_0";
  (*names)[1] = "c_0";
}
...
template <typename Dtype>
void LSTMTopicLayer<Dtype>::RecurrentOutputBlobNames(vector<string>* names) const {
  names->resize(2);
...
template <typename Dtype>
void LSTMTopicLayer<Dtype>::RecurrentInputShapes(vector<BlobShape>* shapes) const {
...
template <typename Dtype>
void LSTMTopicLayer<Dtype>::OutputBlobNames(vector<string>* names) const {
  names->resize(1);
  (*names)[0] = "h";
}

template <typename Dtype>
void LSTMTopicLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  const int num_output = this->layer_param_.recurrent_param().num_output();
  ...
  if (this->topic_input_) {
    // Add layer to transform x_topic to the gate dimension.
    //     W_xc_x_topic = W_xc_topic * x_topic
    LayerParameter* x_topic_transform_param = net_param->add_layer();
    x_topic_transform_param->CopyFrom(hidden_param);
    x_topic_transform_param->mutable_inner_product_param()->set_axis(1);
    x_topic_transform_param->set_name("W_xc_x_topic");
    x_topic_transform_param->add_param()->set_name("W_xc_topic");
    x_topic_transform_param->add_bottom("x_topic");
    x_topic_transform_param->add_top("W_xc_x_topic_preshape");
    x_topic_transform_param->add_propagate_down(true);

    LayerParameter* reshape_topic_param = net_param->add_layer();
    reshape_topic_param->set_type("Reshape");
    BlobShape* new_topic_shape =
         reshape_topic_param->mutable_reshape_param()->mutable_shape();
    new_topic_shape->add_dim(1);  // One timestep.
    // Should infer this->N as the dimension so we can reshape on batch size.
    new_topic_shape->add_dim(-1);
    new_topic_shape->add_dim(
        x_topic_transform_param->inner_product_param().num_output());
    reshape_topic_param->set_name("W_xc_x_topic_reshape");
    reshape_topic_param->add_bottom("W_xc_x_topic_preshape");
    reshape_topic_param->add_top("W_xc_x_topic");
  }
  ...
  for (int t = 1; t <= this->T_; ++t) {
    ...
    //                   = W_hc_h_{t-1} + W_xc_x_t + b_c
    {
      ...
      if (this->topic_input_) {
        input_sum_layer->add_bottom("W_xc_x_topic");
      }
...
INSTANTIATE_CLASS(LSTMTopicLayer);
REGISTER_LAYER_CLASS(LSTMTopic);
