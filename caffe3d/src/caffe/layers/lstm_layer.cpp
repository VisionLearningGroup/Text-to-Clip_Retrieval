...
void LSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  for (int t = 1; t <= this->T_; ++t) {
    ...
    //         h_t := o_t .* \tanh[c_t]
    {
      ...
      //switch (this->layer_param_.recurrent_param().operation()) {
      //  case RecurrentParameter_LSTMActOp_LINEAR:
      //    lstm_unit_param->set_type("LSTMUnit1");
      //    break;
      //  case RecurrentParameter_LSTMActOp_RELU:
      //    lstm_unit_param->set_type("LSTMUnit2");
      //    break;
      //  case RecurrentParameter_LSTMActOp_TANH:
      //  default:
