...
void ROIPoolingEachLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ROIPoolingEachLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void ROIPoolingEachLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < num_rois; ++n) {
    ...
    int roi_end_w = width_-1;
    int roi_end_h = height_-1;
...
template <typename Dtype>
void ROIPoolingEachLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(ROIPoolingEachLayer);
#endif

INSTANTIATE_CLASS(ROIPoolingEachLayer);
REGISTER_LAYER_CLASS(ROIPoolingEach);
