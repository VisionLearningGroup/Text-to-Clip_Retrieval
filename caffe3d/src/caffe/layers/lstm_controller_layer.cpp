...
void LSTMControllerLayer<Dtype>::RecurrentInputBlobNames(vector<string>* names) const {
  names->resize(2);
  (*names)[0] = "h_0";
  (*names)[1] = "c_0";
}
...
template <typename Dtype>
void LSTMControllerLayer<Dtype>::RecurrentOutputBlobNames(vector<string>* names) const {
  names->resize(2);
...
template <typename Dtype>
void LSTMControllerLayer<Dtype>::RecurrentInputShapes(vector<BlobShape>* shapes) const {
...
template <typename Dtype>
void LSTMControllerLayer<Dtype>::OutputBlobNames(vector<string>* names) const {
  names->resize(1);
  (*names)[0] = "h";
}

template <typename Dtype>
void LSTMControllerLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  const int num_output = this->layer_param_.recurrent_param().num_output();
  ...
  for (int t = 1; t <= this->T_; ++t) {
    ...
    {
      LayerParameter* cont_x_param = net_param->add_layer();
      cont_x_param->CopyFrom(scale_param);
      cont_x_param->set_name("x_conted_" + ts);
      cont_x_param->add_bottom("W_xc_x_" + ts);
      cont_x_param->add_bottom("cont_" + ts);
      cont_x_param->add_top("x_conted_" + ts);
    }
    ...
    //                   = W_hc_h_{t-1} + W_xc_x_t + b_c
    {
      ...
      input_sum_layer->add_bottom("x_conted_" + ts);
      if (this->static_input_) {
...
INSTANTIATE_CLASS(LSTMControllerLayer);
REGISTER_LAYER_CLASS(LSTMController);
