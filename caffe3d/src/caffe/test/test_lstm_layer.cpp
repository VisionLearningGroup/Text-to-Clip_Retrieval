...
TYPED_TEST(LSTMLayerTest, TestLSTMUnit1SetUp) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  LSTMUnit1Layer<Dtype> layer(layer_param);
  layer.SetUp(this->unit_blob_bottom_vec_, this->unit_blob_top_vec_);
  const int num_axes = this->unit_blob_bottom_c_prev_.num_axes();
  ASSERT_EQ(num_axes, this->unit_blob_top_c_.num_axes());
  ASSERT_EQ(num_axes, this->unit_blob_top_h_.num_axes());
...
TYPED_TEST(LSTMLayerTest, TestLSTMUnit1Gradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  LSTMUnit1Layer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
...
TYPED_TEST(LSTMLayerTest, TestLSTMUnit1GradientNonZeroCont) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  LSTMUnit1Layer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
...
TYPED_TEST(LSTMLayerTest, TestLSTMUnit2SetUp) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  LSTMUnit2Layer<Dtype> layer(layer_param);
  layer.SetUp(this->unit_blob_bottom_vec_, this->unit_blob_top_vec_);
  const int num_axes = this->unit_blob_bottom_c_prev_.num_axes();
  ASSERT_EQ(num_axes, this->unit_blob_top_c_.num_axes());
  ASSERT_EQ(num_axes, this->unit_blob_top_h_.num_axes());
...
TYPED_TEST(LSTMLayerTest, TestLSTMUnit2Gradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  LSTMUnit2Layer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
...
TYPED_TEST(LSTMLayerTest, TestLSTMUnit2GradientNonZeroCont) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  LSTMUnit2Layer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
...
TYPED_TEST(LSTMLayerTest, TestGradient1) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  RecurrentParameter* recurrent_param = layer_param.mutable_recurrent_param();
  recurrent_param->set_operation(RecurrentParameter_LSTMActOp_LINEAR);
  LSTMLayer<Dtype> layer(this->layer_param_);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
  checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
      this->blob_top_vec_, 0);
...
TYPED_TEST(LSTMLayerTest, TestGradient1NonZeroCont) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  RecurrentParameter* recurrent_param = layer_param.mutable_recurrent_param();
  recurrent_param->set_operation(RecurrentParameter_LSTMActOp_LINEAR);
...
TYPED_TEST(LSTMLayerTest, TestGradient1NonZeroContBufferSize2) {
  typedef typename TypeParam::Dtype Dtype;
  this->ReshapeBlobs(2, 2);
  FillerParameter filler_param;
  UniformFiller<Dtype> filler(filler_param);
...
TYPED_TEST(LSTMLayerTest, TestGradient1NonZeroContBufferSize2WithStaticInput) {
  typedef typename TypeParam::Dtype Dtype;
  this->ReshapeBlobs(2, 2);
  FillerParameter filler_param;
  UniformFiller<Dtype> filler(filler_param);
...
TYPED_TEST(LSTMLayerTest, TestGradient2) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  RecurrentParameter* recurrent_param = layer_param.mutable_recurrent_param();
  recurrent_param->set_operation(RecurrentParameter_LSTMActOp_RELU);
  LSTMLayer<Dtype> layer(this->layer_param_);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
  checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
      this->blob_top_vec_, 0);
...
TYPED_TEST(LSTMLayerTest, TestGradient2NonZeroCont) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  RecurrentParameter* recurrent_param = layer_param.mutable_recurrent_param();
  recurrent_param->set_operation(RecurrentParameter_LSTMActOp_RELU);
...
TYPED_TEST(LSTMLayerTest, TestGradient2NonZeroContBufferSize2) {
  typedef typename TypeParam::Dtype Dtype;
  this->ReshapeBlobs(2, 2);
  FillerParameter filler_param;
  UniformFiller<Dtype> filler(filler_param);
...
TYPED_TEST(LSTMLayerTest, TestGradient2NonZeroContBufferSize2WithStaticInput) {
  typedef typename TypeParam::Dtype Dtype;
  this->ReshapeBlobs(2, 2);
  FillerParameter filler_param;
  UniformFiller<Dtype> filler(filler_param);
